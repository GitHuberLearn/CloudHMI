...
HttpUtils.request = {
    postNotParam: function (url, successCall, errorCall) {
        this.netOfAsync(url, "POST", true, false, false, false, successCall, errorCall);
    },
    getJson: function (url, data, successCall, errorCall) {
        this.netOfAsync(url, "GET", data, true, false, false, successCall, errorCall);
    },
    ...
    postJsonSync: function (url, data, successCall, errorCall) {
        this.netOfsync(url, "POST", data, false, false, false, successCall, errorCall);
    },
    postParamSync: function (url, data, successCall, errorCall) {//同步param请求
        this.netOfsync(url, "POST", data, false, false, true, successCall, errorCall);
    },
    ...
    netOfsync: function (url, method, data, isProgress, isCubeObj, isParam, successCall, errorCall) {
        this.netOfparamVerification(url, method, false, data, isProgress, isCubeObj, true, isParam, successCall, errorCall);
    },
    ...
    netOfparamVerification: function (url, method, async, data, isProgress, isCubeObj, isHaveToken, isParam, successCall, errorCall) {
        ...
        this.netOfBusiness(url, method ? method.toUpperCase() : "GET", async, data, isProgress, isCubeObj, isHaveToken, isParam, successCall, errorCall);
    },
    netOfBusiness: function (url, method, async, data, isProgress, isCubeObj, isHaveToken, isParam, successCall, errorCall) {	// POST
        // @RequestBody
        // 接受参数
        ...
        this.netOfToken(url, method, async, data, isProgress, isHaveToken, isParam, function (dataResult, textStatus, request) {
            ...
            if (isCubeObj) {
                ...
                if (dataResult.successful) {
                    ...
                    }) : HttpUtils.cuTosi(dataResult.resultHint ? dataResult.resultHint : "获取业务数据失败");
    ...
    netOfToken: function (url, method, async, data, isProgress, isHaveToken, isParam, success, error) {	// POST
        // @RequestBody
        // 接受参数
        isProgress ? HttpUtils.loading(0) : "";
    ...
    netOfprogressBar: function (url, method, async, data, isProgress, isParam, success, error, beforecall) {	// POST
        // @RequestBody
        // 接受参数
        isProgress ? HttpUtils.loading(0) : "";
    ...
    netOfJsonParam: function (url, method, async, data, isParam, success, error, beforecall) {	// POST
        // @RequestBody
        // 接受参数
        ...
        isParam ? this.ajax(url, method, async, false, null, data, null, success, error, beforecall) :
            this.ajax(url, method, async, false, "application/json;charset=utf-8", ko.toJSON(data), null, success, error, beforecall);
    },
    ...
    postBody: function (url, data, success, error) {	// POST @RequestBody 接受参数
        this.ajax(url, "POST", null, null, "application/json;charset=utf-8", data, null, success, error);
    },
    ...
    defaultAjax: function (url, method, async, data, success, error) {	// 默认的AJAX配置
        // 请求
...
CUBE = function () {
     ...
     * 获取相似资源列表
     */
    self.queryResAlike = function (resId, resName, viewModel, Zwsj, fgnumber, numbers, dataType) {
        $("#" + Zwsj).hide();
        var postData = {
            "resId": resId,
            "resName": resName,
        ...
        HttpUtils.request.postJson(cube.getResource("/resource/queryResAlike"), postData, function (result) {
            //debugger
    ...
    self.deleteRes = function (viewModel, orderId, resId, resType) {
        ...
        layer.confirm('确认要删除吗？', { title: "<span>信息</span>", shadeClose: true, offset: '100px' }, function (index) {
            HttpUtils.request.postBody(cube.gatewayURL_resource + "/deleteData/deleteResById", JSON.stringify(postData), function (result) {
    ...
    self.deleteOrder = function (viewModel, orderId, resId, resType) {
        ...
        layer.confirm('确认要删除吗？', { title: "<span>信息</span>", shadeClose: true, offset: '100px' }, function (index) {
            HttpUtils.request.postBody(cube.gatewayURL_resource + "/deleteData/deleteOrderById", JSON.stringify(postData), function (result) {
                var resultData = ko.mapping.toJS(result);
                if (resultData.successful == true) {
                    layer.msg('已删除!', {
                        icon: 6,
    ...
    self.myBrowser = function () {
        ...
        var isIE = userAgent.indexOf("Trident") > -1
            && userAgent.indexOf("rv") > -1 && !isOpera; //判断是否IE浏览器
    ...
    self.prewFile = function (data) {
        debugger
        if (data.suffix.toLowerCase() == "ceb" || data.suffix.toLowerCase() == "cebx") {
            var isIE = cube.myBrowser();
            if (isIE == "IE") {
                var url = cube.gatewayURL_file + "/file/returnCebStream?id=" + data.fileId;
                x_admin_show("预览 --- " + data.fileName, url, 'define', '808px', '518px')
                ...
                layer.open({
                    ...
                    area: ['420px', '240px'], // 宽高
                    content: '<div class="container-browser">\
            ...
            var url = cube.gatewayURL_dataPortals + "/home/view/grabble/pdfSelfPreview.html?id=" + data.fileId;
            x_admin_show("预览 --- " + data.fileName, url, 'define', '808px', '518px')
        } else {
            var url = cube.gatewayURL_dataPortals + "/home/view/grabble/pdfpreview.html?id=" + data.fileId + '&' + encodeURI(data.fileName);
            x_admin_show("预览 --- " + data.fileName, url, 'define', '808px', '518px')
        }
    }
    /**
    ...
    self.showPageControl = function (viewModel, myClass) {
        ...
        // 显示分页功能
        if (pageCount <= 0) {
            $("." + pageClass).find('ul').html("");
            return;
        }
        $("." + pageClass).find('ul').html("");
        $("." + pageClass).find('ul').append("<li id='pre'><a class='Previous'><span><</span></a></li>");
        this.getPageArray(pageIndex, pageCount, pageClass);
        //输入页面 跳转组件
        var bstpage = '\
        	<li class="bstpage"><a>\
			    <span class="bst-skip">\
			        到第<input type="text" min="1" value=' + pageIndex + ' class="bst-input">页\
			        <button type="button" class="btn bst-page">确定</button>\
			    </span>\
	    		<span>共' + totalCount + ' 条</span></a>\
			</li>';
        $("." + pageClass).find('ul').append("<li id='next'><a class='Next'><span>></span></a></li>" + bstpage);
        // 添加点击事件
        $("." + pageClass).find('li').on("click", function () {
            var id = $(this).attr("id");
    ...
    self.getPageArray = function (pageIndex, pageCount, pageClass) {
        if (pageCount <= this.pageLimitSize) {
            ...
            if (pageIndex <= this.pageLimitSize - 1) {	// 前半部分
                ...
                $("." + pageClass).find('ul').append(" <li><a >...</a></li>")
                $("." + pageClass).find('ul').append(" <li id='" + (pageCount) + "'><a>" + (pageCount) + "</a></li>")
            } else if (pageCount - pageIndex + 1 <= this.pageLimitSize - 1) {// 后半部分
                $("." + pageClass).find('ul').append(" <li id='1'><a >1</a></li>")
                $("." + pageClass).find('ul').append(" <li><a>...</a></li>")
                this.appendHtml(pageCount - (this.pageLimitSize - 2), pageCount, pageIndex, pageClass)
            } else {
                $("." + pageClass).find('ul').append(" <li id='1'><a >1</a></li>")
                $("." + pageClass).find('ul').append(" <li><a>...</a></li>")
                if (self.pageLimitSize > 4 && self.pageLimitSize % 2 != 0) {
                    this.appendHtml(parseInt(pageIndex) - Math.ceil((this.pageLimitSize - 3) / 2), parseInt(pageIndex) + Math.ceil((this.pageLimitSize - 3) / 2), pageIndex, pageClass);
                } else {
                    this.appendHtml(parseInt(pageIndex) - Math.ceil((this.pageLimitSize - 3) / 2), parseInt(pageIndex) + Math.ceil((this.pageLimitSize - 3) / 2) - 1, pageIndex, pageClass);
                }
                $("." + pageClass).find('ul').append(" <li><a >...</a></li>")
                $("." + pageClass).find('ul').append(" <li id='" + (pageCount) + "'><a>" + (pageCount) + "</a></li>")
            }
